       ...
       <div class="box">
            ...
            <div class="box-body">
              ...
              $q = $_POST['date_range'];
                ...
                $query4 = "select users.name tname from dtr inner join users on users.rf_id = dtr.rf_id order by tname";

                $query5 = ("select users.user_id user_id,users.name fname, to_char(transact_date,'Day') as day,to_char(timein,'HH:MI AM') as timein,to_char(timeout,'HH:MI AM') as timeout,round(extract (epoch from ((timeout::time - timein::time)/3600))::numeric,2) as subtotal from dtr
                INNER JOIN users ON users.rf_id=dtr.rf_id 
                where transact_date between '$from2'::timestamp::date and '$to2'::timestamp::date group by users.user_id, users.name,dtr.transact_date,dtr.timein,dtr.timeout order by users.name;");
